...
def get_args():
    ...
    parser.add_argument('--structure_model_path',
                        help="The path to the structure model")
...
def refine_table_structure(table_structure, class_thresholds):
    ...
    spanning_cells = postprocess.align_supercells(
...
def objects_to_structures(objects, tokens, class_thresholds):
    ...
    for table in tables:
        table_objects = [obj for obj in objects if iob(
            obj['bbox'], table['bbox']) >= 0.5]
        table_tokens = [token for token in tokens if iob(
            token['bbox'], table['bbox']) >= 0.5]

        structure = {}

        columns = [obj for obj in table_objects if obj['label']
                   == 'table column']
        ...
        table['row_column_bbox'] = [column_rect[0],
                                    row_rect[1], column_rect[2], row_rect[3]]
...
class TableExtractionPipeline(object):
    def __init__(self, det_device=None, str_device=None,
        ...
        self.det_class_idx2name = {v: k for k,
                                   v in self.det_class_name2idx.items()}
        self.det_class_thresholds = detection_class_thresholds

        self.str_class_name2idx = get_class_map('structure')
        self.str_class_idx2name = {v: k for k,
                                   v in self.str_class_name2idx.items()}
    ...
    def detect(self, img, tokens=None, out_objects=True, out_crops=False, crop_padding=10):
        ...
        objects = outputs_to_objects(
            outputs, img.size, self.det_class_idx2name)
        if out_objects:
    ...
    def recognize(self, img, tokens=None, out_objects=False, out_cells=False,
        ...
        # Post-process detected objects, assign class labels
        objects = outputs_to_objects(
            outputs, img.size, self.str_class_idx2name)
        if out_objects:
            out_formats['objects'] = objects
        ...
        tables_cells = [structure_to_cells(structure, tokens)[
            0] for structure in tables_structure]
        if out_cells:
...
def process_bbox(bbox, w, h):
    x_min = min([point[0] for point in bbox])
    y_min = min([point[1] for point in bbox])
    x_max = max([point[0] for point in bbox])
    y_max = max([point[1] for point in bbox])

    x_min = max(0, int(x_min)-1)
    y_min = max(0, int(y_min)-1)
    x_max = min(w, int(x_max)+1)
    y_max = min(h, int(y_max)+1)
...
def main():
    ...
    for count, img_file in enumerate(img_files):
        ...
        print("{} ({}/{})".format(img_file, count+1, num_files))
        ...
        if not args.words_dir is None:
            tokens_path = os.path.join(
                args.words_dir, img_file.replace(".jpg", "_words.json"))
            ...
            if result[0] is not None:
                ...
                for i, res in enumerate(result[0]):
                    ...
                    tmp = {}
                    tmp["text"] = pair[0]
                    tmp["bbox"] = process_bbox(bbox, w, h)
                    tmp["span_num"] = i
                    tmp["line_num"] = 0
                    tmp["block_num"] = 0
                    tokens.append(tmp)
